<script lang="ts">
	import { Heading, Text, Breadcrumb, Accordion, Card, Button } from "atomic-design-svelte";
...
<section class="bg-surface-tertiary py-8 sm:py-12">
	<div class="container mx-auto px-4 sm:px-6">
		...
		<Heading level="h1" class="mt-4 mb-2 text-2xl sm:text-3xl md:text-4xl">Preguntas Frecuentes</Heading>
	...
	<div class="container mx-auto px-4 sm:px-6">
		<div class="space-y-6 sm:space-y-8">
			{#each faqCategories as category}
				<div>
					<Heading level="h2" class="mb-4 sm:mb-6 text-xl sm:text-2xl">{category.title}</Heading>
					<Accordion>
						{#snippet items()}
							{#each category.items as item}
								<div>
									<button class="accordion-trigger w-full text-left p-4 bg-surface-card border border-border-default rounded-lg hover:bg-surface-tertiary transition-colors flex items-center justify-between">
										<Text class="font-semibold">{item.question}</Text>
										<svg class="w-5 h-5 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
											<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
										</svg>
									</button>
									<div class="accordion-content p-4 bg-surface-tertiary rounded-lg mt-2">
										<Text class="text-text-muted">{item.answer}</Text>
									</div>
								</div>
							{/each}
						{/snippet}
		...
		<Card padding="lg" class="mt-12 text-center">
			{#snippet children()}
				<Heading level="h3" class="mb-4">¿No encuentras tu respuesta?</Heading>
				...
				<div class="flex flex-col sm:flex-row gap-4 justify-center">
					...
					<Text class="self-center text-text-muted">
						o llámanos: <Link href="tel:+34XXXXXXXXX" class="text-primary hover:underline font-semibold">+34 XXX XXX XXX</Link>
					</Text>
				</div>
			{/snippet}
		</Card>
