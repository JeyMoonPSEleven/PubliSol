...
<script lang="ts">
	...
	let repeatedItems = $derived(
		items.length > 0 ? Array(REPEAT_COUNT).fill(items).flat() : [],
	);
			...
			<div class="flex-shrink-0">
				...
				>
					...
					{#if item.tags && item.tags.length > 0}
						<div
							class="absolute top-2 left-2 right-2 z-10 flex flex-wrap gap-1.5"
						>
							{#each item.tags as tag}
								<span
									class="rounded-full bg-white/95 backdrop-blur-sm px-2 py-1 text-[10px] font-medium text-gray-700 shadow-sm"
								>
									{tag}
								</span>
							{/each}
					...
					<div
						class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/60 to-transparent p-3 sm:p-4"
					>
						<h3
						...
						{#if item.subtitle}
							<p
								class="text-xs sm:text-sm text-white/90 line-clamp-1"
							>
								{item.subtitle}
							</p>
						{/if}
...
<style>
	...
	@keyframes scroll-up {
		...
		100% {
			transform: translateY(calc(-100% / var(--repeat-count, 3)))
				translateZ(0);
		}
	}
